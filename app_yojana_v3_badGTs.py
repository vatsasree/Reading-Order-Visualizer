...
# Load the JSON data
# with open('/home/vatsasree/Research/scripts/applic/Reading-Order-Visualizer/jsons/yojana/dic_yojana_chebyshev_no_margins_yolo.json', 'r') as json_file:
# with open('/home/vatsasree/Research/scripts/applic/Reading-Order-Visualizer/temp_jsonsfromada/dic_yojana_chebyshev_no_margins_yolo.json','r') as json_file: 
with open('/home/vatsasree/Research/scripts/applic/Reading-Order-Visualizer/temp_jsonsfromada/dic_yojana_chebyshev_no_margins_yolo_v2layout_v2algo_bwf_v3doctr_yojana_badGTs.json','r') as json_file:   
...
def final_orderr():
    ...
    font_size = request.args.get('font_size', default=0.1, type=float)
...
# # Assuming load_image, ignore_margins, get_coordinates_from_component are defined elsewhere
...
# @lru_cache(maxsize=128)
# def cached_get_component_data(image_sp):
#     return image_data.get(image_sp, {}).get('reading_order', {}).get('component')

# @lru_cache(maxsize=128)
# def cached_get_new_data(image_sp):
#     return pd.DataFrame(image_data.get(image_sp, {}).get('reading_order', {}).get('new'))

# @lru_cache(maxsize=128)
# def cached_get_euclidean_data(image_sp):
#     return pd.DataFrame(image_data.get(image_sp, {}).get('reading_order', {}).get('euclidean'))
...
#         # component = cached_get_component_data(current_image_sp)
#         component_df = cached_get_new_data(current_image_sp)
#         euclidean_df_2 = cached_get_euclidean_data(current_image_sp)
...
#         # temp_output_path = os.path.join(output_folder, 'output_final_order_image.jpg')
#         # cv2.imwrite(temp_output_path, image_with_para)
#         temp_output_path = os.path.join(output_folder, 'output_final_order_image.webp')
#         cv2.imwrite(temp_output_path, image_with_para, [cv2.IMWRITE_WEBP_QUALITY, 90])  # Save as WebP with quality parameter


#         relative_path = os.path.relpath(temp_output_path, app.config['UPLOAD_FOLDER'])

#         return render_template('index_rp.html', current_image=current_image, image_path='/images/output_images/output_final_order_image.jpg', image_files=image_files, header_p=header_p,
#                                footer_p=footer_p, width_p=width_p, font_size=font_size, font_thickness=font_thickness,
#                                box_thickness=box_thickness, line_thickness=line_thickness)
#     else:
#         return render_template("index_rp.html", current_image=None, image_path=None, image_files=image_files, error_message="Invalid image index")
